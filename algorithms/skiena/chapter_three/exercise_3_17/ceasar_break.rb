...
# Original
message = File.open("text.txt", "r").read.downcase

# Encoded
encoded = message.split("").collect do |char|
...
decoded = encoded.split("").collect do |char|
  if LETTERS.include? char
    LETTERS[(char.ord - ?a.ord - reverse_shift)%LETTERS.size]
  else
    char
  end
end.join
