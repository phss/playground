  ...
  (loop [d (digits number) s ""]
    (let [n (digit-to-number d)] 
      (cond (contains? to-str n) (str s (to-str n))
            (and (= 3 (count d)) (zero? (digit-to-number (rest d)))) (str s (to-str (first d)) "hundred")
            (= 3 (count d)) (recur (rest d) (str s (to-str (first d)) "hundredand"))
            (= 2 (count d)) (recur (rest d) (str s (to-str (* 10 (first d)))))))))
