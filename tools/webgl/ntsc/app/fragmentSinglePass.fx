...
vec4 CompositeSample(vec2 UV) {
	vec2 InverseRes = 1.0 / TextureSize.xy;
	...
	vec3 Texel0 = texture2D(u_image, C0).rgb;
	vec3 Texel1 = texture2D(u_image, C1).rgb;
	vec3 Texel2 = texture2D(u_image, C2).rgb;
	vec3 Texel3 = texture2D(u_image, C3).rgb;
	
	// Calculated the expected time of the sample.
	vec4 T = A * Cy * vec4(TextureSize.x) * Two + B + Cx;
...
vec4 NTSCCodec(vec2 UV)
{
	...
	vec4 IAccum = Zero;
	vec4 QAccum = Zero;
	float QuadXSize = TextureSize.x * 4.0;
	...
	for(float n = -41.0; n < 42.0; n += 4.0)
	{
		...
		vec4 WT = W * (CoordX  + A * CoordY * Two * TextureSize.x + B);
...
void main() {
	vec2 InverseRes = 1.0 / TextureSize.xy;
	vec2 UV = v_texCoord.xy * InverseRes;

	vec4 OutPixel = NTSCCodec(v_texCoord.xy);
